...
fn diff_deleted(
  ...
  case map.has_key(curr, key) {
    True -> patches
    False -> Patches(..patches, deleted: [DeletedNode(key), ..patches.deleted])
  }
}
...
pub fn encode_patches(patches: Patches) -> Json {
  json.object([
    #("$", json.string("Patches")),
    #("created", json.array(patches.created, encode_created_patch)),
    #("deleted", json.array(patches.deleted, encode_deleted_patch)),
  ])
}

fn encode_created_patch(patch: CreatedPatch) -> Json {
  case patch {
    CreatedNode(key, t, params) ->
      json.object([
        #("$", json.string("CreatedNode")),
        #("key", json.string(key)),
        #("type", json.string(t)),
        #("params", json.array(params, param.encode)),
      ])
    CreatedParam(key, name, value) ->
      json.object([
        #("$", json.string("CreatedParam")),
        #("key", json.string(key)),
        #("name", json.string(name)),
        #("value", json.float(value)),
      ])
    CreatedProperty(key, name, value) ->
      json.object([
        #("$", json.string("CreatedProperty")),
        #("key", json.string(key)),
        #("name", json.string(name)),
...
fn encode_deleted_patch(patch: DeletedPatch) -> Json {
  case patch {
    DeletedNode(key) ->
      json.object([
        #("$", json.string("DeletedNode")),
        #("key", json.string(key)),
      ])
    DeletedParam(key, name) ->
      json.object([
        #("$", json.string("DeletedParam")),
        #("key", json.string(key)),
        #("name", json.string(name)),
      ])
    DeletedProperty(key, name) ->
      json.object([
        #("$", json.string("DeletedProperty")),
        #("key", json.string(key)),
        #("name", json.string(name)),
      ])
